...
def prepare_money_plot(g1s, orig_ids, scats, fnc, fit_procedure=chi_sq_fit, N=1000, model=linear_f, args_iso=[], args_grp=[]):
    ...
    # #make sure catalogues are sorted and intersected. 
    # scats = maps.get_intersection_cats(scats)
    # fcats=[]
    # for cat in scats: 
    #     cat.sort(maps.dflt_sorting) #deepcopy cat
    #     fcats.append(cat)
    # scats = fcats
...
def errs_param_indp_boot(cats, param, N, func):
    stds = []
    for cat in cats:
...
#enter params = ['params1', 'params2',....]
#get [(param11,param21,...), (param12,param21,....), ...] with N elements.
def bootstrap_params_cat(cats, params, N):
    ids = list(cats[0]['db_id'])
    bootstrapped_params = [[] for i in range(len(cats))]
    n = len(ids)
    hash_maps = [[hash_param(cat, param) for param in params] for cat in cats] #get a hash map of param for cats for quick look up

    # for each cat produce a bootstrapped sample of N funcs
    for _ in range(N):
        id_sample = np.random.choice(ids,size=n)
        for i, cat in enumerate(cats):
            bootstrapped_params[i].append( [tuple(hash_map[gal_id] for hash_map in hash_maps[i]) for gal_id in id_sample] )
    return bootstrapped_params
...
#the output is len(cats) catalogues of length L, each of the boostrapped catalogues has the same ids.
# the output ALWAYS has the same size as the original catalogue(s).
#The input ALWAYS has to have distinct, unique ids (no duplicates)
def bootstrap_cats(cats, N):
    ...
    for i in range(N):
        ...
        boot_cats.append([Table(rows=rows,names=rows[0].colnames) for rows in boot_rows])
