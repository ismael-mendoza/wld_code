...
#     os.system('python all-process.py --simulate-all --num-sections 10 --cosmic-shear-g1 {0} --cosmic-shear-g2 0 --project {3}{2}-g1_{1}-g2_0 --survey-name {2}'.format(g,int(float(g)*1000),survey_name, project_name))
...
#for g in ['0.0']: 
for g in ['-0.1','-0.05','-.02','-.015', '-.01', '-.005','.005','.01','.015', '0.02', '0.05', '0.1']: 
    os.system('bsub -W 30:00 -o "output-{2}{1}-g1_{0}-g2_0.txt" -r "python all-process.py --process-all --add-noise-all --extract-all --combine  --num-sections 10 --project {2}{1}-g1_{0}-g2_0 --noise-seed 0 --survey-name {1}" '.format(int(float(g)*1000),survey_name,project_name))
