...
def prepare_money_plot(g1s, orig_ids, scats, fnc, fit_procedure=chi_sq_fit, N=1000, model=linear_f, args_iso=[], args_grp=[]):
    ...
    return (values, values_grp), (cov, cov_grp, inv, inv_grp, corr, corr_grp), (errs, errs_grp), (boot_values, boot_values_grp)


def make_money_plot(g1s, values, errs, values_grp, errs_grp, inv, inv_grp, fit_procedure=chi_sq_fit, model=linear_f, ticks1=None, labely1=None, ax = None, colors=None, extra=None,
    ...
    * For simplicity this function only supports money plot for g1 component on bias. 
    ...
    ax.errorbar(g1s,values,yerr=errs,marker='o',linestyle=' ',color=colors[0],capsize=3,label="\\rm Blending off{}".format(extra) )
    ax.errorbar(g1s,values_grp,yerr=errs_grp,marker='o',linestyle=' ',color=colors[1],capsize=3,label="\\rm Blending on{}".format(extra) )
    ...
    if model is linear_f: 
        beta0, beta1 = betas 
        (beta0_err,beta1_err), beta01_corr = np.sqrt(np.diag(param_cov)), param_cov[0,1]/np.sqrt(param_cov[0,0]*param_cov[1,1])
        beta0_grp, beta1_grp = betas_grp
        (beta0_err_grp,beta1_err_grp), beta01_corr_grp = np.sqrt(np.diag(param_cov_grp)), param_cov_grp[0,1]/np.sqrt(param_cov_grp[0,0]*param_cov_grp[1,1])
